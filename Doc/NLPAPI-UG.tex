...
\begin{document}
  ...
  \maketitle
   ...
   \section{Introduction}
    ...
    \begin{displaymath}
     \begin{array}{ll}
      ...
      \qquad c_i({\bf x}) = 0& \qquad\qquad \rm Nonlinear~Equality
                                                      ~Constraints\\
      \qquad L_i \leq c_i({\bf x}) \leq U_i& \qquad\qquad \rm Nonlinear
   ...
   \end{displaymath}
    ...
    empty, and the user then sets the objective function, and add equality and inequality constraints.

   \subsection{Functions}

    The problem consists of functions $F({\bf x})$, which are scalar valued functions of the problem variables ${\bf x}$. 
    In the problem definition there are two functions, the objective function $O({\bf x})$ and constraints $c_i({\bf x})$.
