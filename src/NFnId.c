...
extern "C" {
#endif
int ECNumberOfFunctionIdentifiers(struct ECObjectCode *object, int *rc)
 {
  int i;
  int n;
  ...
  if((*object).successful!=EC_NO_ERROR)
   {
    if(ECPrintErrorMessages)fprintf(stderr,"ECNumberOfFunctionIdentifiers: compilation was unsuccessful\n");
  ...
  if((*object).nSymbols>0)
   {
    for(i=0;i<(*object).nSymbols;i++)
     {
      if((*object).symbolType[i]==ECFunction)n++;
     }
   }
